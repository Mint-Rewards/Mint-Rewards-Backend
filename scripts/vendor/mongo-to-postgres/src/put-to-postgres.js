// Vendored + patched from mongo-to-postgres 0.0.6 — see ../NOTICE.md.
// PATCH: arrays not declared in `links` are JSON.stringify'd into the row
// instead of being deleted, so they survive as jsonb (upstream drops them
// with no way to opt out). See the "remove/serialize arrays" block below.
...
export default async ({ knex, collections, tableName, rows }) => {
  ...
  const linkFieldNames = new Set(Object.keys(links || {}));
  ...
  for (const currentRow of rows) {
    ...
    // remove arrays handled by `links` (they're inserted into a join table
    // below); serialize any other array to JSON so it survives as jsonb
