...
14-day dual-write cutover needs a Mongo-id ↔ Postgres-row link, and every live
...
- [~] **Query parity against the actual app.** The real acceptance test is
      ...
      **Baseline CAPTURED 2026-09-03** — `baseline-mongoose.json` in the repo
      root (3.0 MB, 141 interactions, `complete: true`, 652 ids symbolised),
      committed on `feature/postgres-migration-rehearsal`. It was safety-scanned
      before use: 0 JWTs, 0 bcrypt hashes, 0 plaintext passwords.
      **Still to do:** re-capture needs `ADMIN_PASSWORD`,
