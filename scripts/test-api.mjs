...
function log(label, status, detail = "") {
  ...
  console.log(
    `  ${icon} ${colour}${label}${RESET}${detail ? `  ${DIM}${detail}${RESET}` : ""}`,
  );
...
async function testRegistration() {
  ...
  // 1. Missing required fields
  {
    ...
    const { status, data } = await call("POST", "/brands/register", {
      form: fd,
    });
  ...
  if (BRAND_ID && registeredRegNumber) {
    ...
    if (status === 409) {
      ...
      log(
        "Duplicate reg number returns 409",
        "FAIL",
        `status=${status} msg="${data.message}"`,
      );
    }
...
async function testBrandAdmin() {
  ...
  // Approve brand
  {
    ...
    if (status === 200 && data.brand?.status === "APPROVED") {
      ...
      log(
        "Approve brand",
        "FAIL",
        `status=${status} brandStatus=${data.brand?.status}`,
      );
    }
  }
  ...
  // Re-approve (needed for subsequent tests)
  {
    ...
    if (status === 200) {
      ...
      log(
        "Re-approve brand for subsequent tests",
        "FAIL",
        `status=${status} msg="${data.error}"`,
      );
    }
  ...
  // Valid partial update
  {
    ...
    if (status === 200 && data.brand) {
      log(
        ...
        "PASS",
        `themeColor=${data.brand.themeColor}`,
      );
    } else {
      log(
        "Partial settings update",
...
async function testCampaigns() {
  ...
  // POST — minimal (name only)
  {
    ...
    if (status === 201 && data.campaign?._id) {
      ...
      log(
        "POST minimal campaign",
        "PASS",
        `id=${CAMPAIGN_ID} status=${data.campaign.status}`,
      );
    } else {
      log(
        "POST minimal campaign",
  ...
  // POST — with banner (multipart)
  {
    ...
    if (status === 201 && data.campaign?.banner) {
      log(
        "POST campaign with banner (multipart)",
        "PASS",
        `bannerUrl=${data.campaign.banner.slice(0, 60)}…`,
      );
    } else {
      log(
        "POST campaign with banner (multipart)",
  ...
  // PATCH — brand fields
  {
    const { status, data } = await call(
      "PATCH",
      `/brands/${HUB_BRAND_ID}/campaigns/${CAMPAIGN_ID}`,
      {
        body: {
  ...
  // gates the row, then the "status" field additionally requires admin)
  {
    ...
    if (status === 200 && data.campaign?.status === "APPROVED") {
      ...
      log(
        "PATCH approve campaign (admin)",
        "FAIL",
        `status=${status} campaignStatus=${data.campaign?.status}`,
      );
    }
  }
...
async function testDeals() {
  ...
  // POST — minimal
  {
    ...
    if (status === 201 && data.deal?._id) {
      ...
      log(
        "POST minimal deal",
        "PASS",
        `id=${DEAL_ID} status=${data.deal.status}`,
      );
    } else {
      log(
        "POST minimal deal",
  ...
  // PATCH — update content fields
  {
    const { status, data } = await call(
      "PATCH",
      `/brands/${HUB_BRAND_ID}/deals/${DEAL_ID}`,
      {
        body: {
...
async function testAdminViews() {
  ...
  for (const s of ["PENDING", "APPROVED", "REJECTED", "EXPIRED"]) {
    const { status, data } = await call(
      "GET",
      `/brands/campaigns?status=${s}`,
      { headers: adminHeaders() },
    );
    if (status === 200 && Array.isArray(data.campaigns)) {
      log(
        `GET /brands/campaigns?status=${s}`,
        "PASS",
        `count=${data.campaigns.length}`,
      );
    } else {
  ...
  if (BRAND_ID) {
    const { status, data } = await call(
      "GET",
      `/brands/campaigns?brandId=${BRAND_ID}`,
      { headers: adminHeaders() },
    );
    if (status === 200 && Array.isArray(data.campaigns)) {
      log(
        "GET /brands/campaigns?brandId=…",
        "PASS",
        `count=${data.campaigns.length}`,
      );
    } else {
...
async function registerAppUser() {
  ...
  if (status === 200 && data.token) {
    ...
    log(
      "App user signup",
      "PASS",
      `mintId=${data.user?.mintId} points=${data.user?.points}`,
    );
  } else {
...
async function testAdminLoginNegative() {
  ...
  // Unknown email → 401
  {
    const { status } = await call("POST", "/admin/login", {
      body: {
        email: `nobody${Date.now()}@nowhere.com`,
...
async function testUserSignup() {
  ...
  // GET liveness probe
  {
    ...
    if (status === 200 && /alive/i.test(data.message || ""))
      log("GET liveness → alive", "PASS");
  ...
  // Password mismatch → 400
  {
    ...
    const { status } = await call("POST", "/users/signup", {
      body: {
        userName: "Mismatch",
        email: `mismatch${unique}@testuser.com`,
        password: "Password123!",
...
async function testUserLogin() {
  ...
  if (USER_EMAIL) {
    ...
    if (
      status === 200 &&
      data.success &&
      String(data.token).startsWith("Bearer ")
    ) {
  ...
  // Login with unknown email returns the same 401 (no user enumeration)
  {
    ...
    if (status === 401) log("Unknown email → 401 (no enumeration)", "PASS");
      ...
      log("Unknown email → 401 (no enumeration)", "FAIL", `status=${status}`);
  }
}
...
async function testPasswordReset() {
  ...
  {
    const { status } = await call("POST", "/users/reset-password", {
      body: {},
    });
  ...
  // reset-password non-string email → 400
  {
    ...
    if (status === 400) log("reset-password non-string email → 400", "PASS");
      ...
      log("reset-password non-string email → 400", "FAIL", `status=${status}`);
  }
  ...
  // reset-password unknown email → 200 generic (no enumeration)
  {
    ...
    if (status === 200 && /if an account exists/i.test(data.message || "")) {
      ...
      log(
        "reset-password unknown email → 200 generic",
        "FAIL",
        `status=${status}`,
      );
    }
  ...
  if (USER_EMAIL) {
    ...
    if (status === 200 && /if an account exists/i.test(data.message || "")) {
      ...
      log(
        "reset-password existing email → 200 generic",
        "FAIL",
        `status=${status}`,
      );
    }
...
async function testUserProfile() {
  ...
  // GET my-profile with a malformed token → 401
  {
    ...
    if (status === 401) log("GET my-profile malformed token → 401", "PASS");
      ...
      log("GET my-profile malformed token → 401", "FAIL", `status=${status}`);
  }
  ...
  {
    const { status, data } = await call("GET", "/users/my-profile", {
      headers: userHeaders(),
    });
    if (status === 200 && data.user && data.user.password === undefined) {
      log(
        "GET my-profile authenticated → 200 (no password)",
        "PASS",
      ...
      log(
        "GET my-profile authenticated → 200 (no password)",
        "FAIL",
        `status=${status} hasPw=${data.user?.password !== undefined}`,
      );
    }
  }
  ...
  {
    const { status } = await call("PUT", "/users/update-profile", {
      body: { userName: "Nope" },
    });
    if (status === 401) log("PUT update-profile without auth → 401", "PASS");
    else
      log("PUT update-profile without auth → 401", "FAIL", `status=${status}`);
  }
  ...
  // PUT update-profile authenticated → 200 + fields applied
  {
    ...
    if (status === 200 && data.userName === "Updated App User") {
      ...
      log(
        "PUT update-profile → 200 (fields applied)",
        "FAIL",
        `status=${status}`,
      );
    }
...
async function testDeleteAccount() {
  ...
  {
    const { status, data } = await call("DELETE", "/users/delete-account", {
      headers: throwawayHeaders,
    });
  ...
  // Deleting again with the same (now-orphaned) token → 404
  {
    ...
    if (status === 404) log("Delete already-deleted account → 404", "PASS");
      ...
      log("Delete already-deleted account → 404", "FAIL", `status=${status}`);
  }
}
...
async function testUserDiscounts() {
  ...
  {
    const { status, data } = await call("GET", "/users/my-discounts", {
      headers: userHeaders(),
    });
    if (status === 200 && Array.isArray(data.discounts)) {
      log(
        "GET my-discounts → 200 array",
        "PASS",
  ...
  {
    const { status } = await call("PATCH", "/users/my-discounts", {
      body: { discountId: "000000000000000000000000" },
      headers: userHeaders(),
    });
...
async function testReferrals() {
  ...
  // Valid fresh emails → 200 success
  {
    ...
    const { status } = await call("POST", "/users/referrals", {
      body: {
        emails: [
          `referral${unique}@friend.com`,
          `referral2${unique}@friend.com`,
        ],
      },
      headers: userHeaders(),
    });
...
async function testActiveCampaigns() {
  ...
  {
    const { status, data } = await call("GET", "/users/active-campaigns", {
      headers: userHeaders(),
    });
    if (
      status === 200 &&
      Array.isArray(data.activeBrands) &&
      Array.isArray(data.activeCampaigns)
    ) {
      log(
        "active-campaigns → 200 arrays",
        "PASS",
...
async function testSocialAuth() {
  ...
  {
    const { status } = await call("POST", "/auth/google", {
      body: { idToken: "clearly-not-a-real-google-token" },
    });
  ...
  {
    const { status } = await call("POST", "/auth/apple", {
      body: { identityToken: "clearly-not-a-real-apple-token" },
    });
    if (status === 401 || status === 500)
      log("apple invalid identityToken → 401/500", "PASS", `status=${status}`);
    else
      log("apple invalid identityToken → 401/500", "FAIL", `status=${status}`);
  }
}
...
async function testBrandhubBrands() {
  ...
  {
    const { status, data } = await call("GET", "/brandhub/brands", {
      headers: brandHeaders(),
    });
    if (status === 200 && Array.isArray(data.brands)) {
      log(
        "GET brandhub brands → 200 array",
        "PASS",
        `count=${data.brands.length}`,
      );
    } else {
  ...
  if (HUB_BRAND_ID) {
    ...
    if (status === 401) log("GET brandhub brand :id no auth → 401", "PASS");
      ...
      log("GET brandhub brand :id no auth → 401", "FAIL", `status=${status}`);
  }
  ...
  // GET brand outside caller's org (nonexistent id) → 404 (scope hides existence)
  {
    const { status } = await call(
      "GET",
      "/brandhub/brands/000000000000000000000000",
      { headers: brandHeaders() },
    );
    if (status === 404)
      log("GET brandhub foreign/nonexistent brand → 404", "PASS");
    else
      log(
        "GET brandhub foreign/nonexistent brand → 404",
        "FAIL",
        `status=${status}`,
      );
  }
  ...
  {
    const { status } = await call("GET", "/brandhub/brands/not-an-object-id", {
      headers: brandHeaders(),
    });
    if (status === 404) log("GET brandhub malformed brand id → 404", "PASS");
    else
      log("GET brandhub malformed brand id → 404", "FAIL", `status=${status}`);
  }
}
...
async function testBrandhubModules() {
  ...
  // Unknown module → 404 (checked before auth, so no token needed)
  {
    const { status } = await call(
      "GET",
      "/brandhub/modules/definitely-not-a-module",
    );
  ...
  {
    const { status } = await call("POST", "/brandhub/modules/bogus", {
      body: {},
      headers: brandHeaders(),
    });
...
async function testCoupons() {
  ...
  // No auth → 401
  {
    const { status } = await call(
      "PATCH",
      "/coupons/000000000000000000000000/redeem",
    );
  ...
  {
    const { status } = await call("PATCH", "/coupons/not-an-object-id/redeem", {
      headers: userHeaders(),
    });
...
async function testLogs() {
  ...
  {
    const { status } = await call("POST", "/logs", {
      body: { event: "app_open" },
    });
  ...
  {
    const { status, data } = await call("GET", "/logs", {
      headers: adminHeaders(),
    });
    if (
      status === 200 &&
  ...
  // GET logs with filters (admin) → 200
  {
    const { status, data } = await call(
      "GET",
      "/logs?event=app_open&level=info",
      { headers: adminHeaders() },
    );
    if (status === 200 && Array.isArray(data.logs)) {
      log(
        "GET logs filtered (admin) → 200",
        "PASS",
